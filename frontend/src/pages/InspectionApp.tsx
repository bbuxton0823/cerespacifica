        ...
        <div className="flex rounded-md shadow-sm" role="group">
          {['L', 'C', 'R'].map((l) => (
            <button
              ...
              className={`px-3 py-1 border border-slate-300 first:rounded-l-md last:rounded-r-md ${location.horizontal === l ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-700 hover:bg-slate-50'
                }`}
            >
              {l}
            </button>
        ...
        <div className="flex rounded-md shadow-sm" role="group">
          {['F', 'C', 'R'].map((l) => (
            <button
              ...
              className={`px-3 py-1 border border-slate-300 first:rounded-l-md last:rounded-r-md ${location.vertical === l ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-700 hover:bg-slate-50'
                }`}
            >
              {l}
            </button>
      ...
      <div className="min-h-screen bg-slate-900 text-white p-6 flex flex-col items-center justify-center relative">
        ...
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="bg-slate-800 p-6 rounded-xl shadow-xl space-y-4">
            ...
            <div>
              <label className="block text-sm text-slate-400 mb-1">T-Code (HUD)</label>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={details.tCode}
                  onChange={e => setDetails({ ...details, tCode: e.target.value })}
                  className="flex-1 bg-slate-900 border border-slate-700 rounded-lg p-3 text-white"
                  placeholder="e.g. T-12345"
                />
                <button onClick={() => handleDictation(v => setDetails({ ...details, tCode: v }))} className="p-3 bg-slate-700 rounded-lg text-white hover:bg-slate-600">
                  {isListening ? <MicActiveIcon /> : <MicIcon />}
                </button>
              </div>
            </div>
            ...
            <div className="pt-6 border-t border-slate-700">
              <h3 className="text-sm font-bold text-slate-400 mb-3 uppercase tracking-wider">Data Tools</h3>
              <div className="grid grid-cols-2 gap-4">
                <label className="flex flex-col items-center justify-center p-4 bg-slate-900 border border-slate-700 border-dashed rounded-lg cursor-pointer hover:bg-slate-800 transition-colors">
                  <i className="fas fa-file-import text-blue-500 text-2xl mb-2"></i>
                  <span className="text-xs text-slate-300">Import CSV</span>
                  <input type="file" className="hidden" accept=".csv,.json" onChange={(e) => {
                    const file = e.target.files?.[0];
                    if (file) {
                      // In a real app, this would call the backend API
                      // For now, we'll just alert
                      alert(`Selected file: ${file.name}. Ready to upload to /api/integrations/import`);
                    }
                  }} />
                </label>
                ...
                <button
                  onClick={() => {
                    ...
                    alert("Downloading export from /api/integrations/export...");
                  }}
                  className="flex flex-col items-center justify-center p-4 bg-slate-900 border border-slate-700 rounded-lg hover:bg-slate-800 transition-colors"
                >
                  <i className="fas fa-file-export text-green-500 text-2xl mb-2"></i>
                  <span className="text-xs text-slate-300">Export Results</span>
                </button>
              </div>
            </div>
          </div>
              ...
              <div className="flex gap-2 mb-2">
                {(['Tenant', 'Owner', 'Owner Representative'] as const).map(role => (
                  <button
                    ...
                    className={`px-3 py-1 text-sm rounded border ${secondSignerRole === role
